...
import (
	...
	"eridiumdev/yandex-praktikum-go-devops/internal/metrics/server/repository"
)

type dummyRenderer struct{}

func (e *dummyRenderer) RenderList(templateName string, data []domain.Metric) ([]byte, error) {
	html := "<html>"
	for i, m := range data {
		if i > 0 {
			html += " | "
...
func getDummyRepo() server.MetricsRepository {
...
func runTests(t *testing.T, tt TestCase) {
	...
	_ = NewMetricsHandler(r, getDummyRepo(), getDummyRenderer())
